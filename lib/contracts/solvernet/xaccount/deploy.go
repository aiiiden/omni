...
// Deploy idempotently deploys a new XAccount contract and returns the address and receipt.
func Deploy(ctx context.Context, network netconf.Network, backend *ethbackend.Backend) (common.Address, *ethtypes.Receipt, error) {
	...
	isDeployed, err := contracts.IsDeployed(ctx, backend, addrs.XAccount)
	if err != nil {
		return common.Address{}, nil, errors.Wrap(err, "is deployed")
	} else if isDeployed {
		return addrs.XAccount, nil, nil
	}
...
func deploy(ctx context.Context, cfg DeploymentConfig, backend *ethbackend.Backend) (common.Address, *ethtypes.Receipt, error) {
	...
	outbox, err := bindings.NewSolverNetOutbox(cfg.Outbox, backend)
	if err != nil {
		return common.Address{}, nil, errors.Wrap(err, "new outbox")
	}

	executor, err := outbox.Executor(&bind.CallOpts{Context: ctx})
	if err != nil {
		return common.Address{}, nil, errors.Wrap(err, "get executor")
	}

	impl, tx, _, err := bindings.DeployXAccount(txOpts, backend, executor)
	if err != nil {
